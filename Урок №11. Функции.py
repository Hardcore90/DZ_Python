...
while num < 1:
    print("Число не является натуральным! Попробуйте еще раз!")
    num = int(input("Введите натуральное число: "))
def factorial(num):
    f = 1
    if num < 2:
...
def create():    
    ...
    for i in fields:
        res = input(f'{i}: ')
        tmp[name][i] = int(res) if res.isnumeric() else res
...
def read():
    ID = int(input("Введите идентификатор питомца, о котором нужно вывести информацию: "))
    pet = get_pet(ID)
    if not pet:
        print("Питомца с таким идентификатором нет в базе!")
    else:
        for k,v in pet.items():
            print(f"Это {v['Вид питомца']} по кличке '{k}'. Возраст питомца: {get_suffix(v['Возраст питомца'])}. Имя владельца: {v['Имя владельца']}")
...
# def update():
#     ID = int(input("Введите идентификатор питомца для обновления информации:"))
#     pet = get_pet(ID)
#     fields = ["Вид питомца", "Возраст питомца", "Имя владельца"]
#     # tmp = {pet.values()[0]: dict()}
#     print(pet[0])
#     for i in fields:
#         res = input(f'{i}: ')
#         tmp[name][i] = int(res) if res.isnumeric() else res
...
def update():
    ID = int(input("Введите идентификатор питомца, о котором нужно обновить информацию: "))
    pet = get_pet(ID)
    if not pet:
        print("Питомца с таким идентификатором нет в базе!")
        return
